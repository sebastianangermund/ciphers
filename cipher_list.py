cipher_list = [
    "mndmknea rse-n-s-m-k nare tfrnenp_senprsercbrnse nprse inc prse nmrse bpre hld _ldncbe ( tfxle tcxl n lbe )",
    "al-prppit xly ppiy ncbe mgkse wcd rcbr nse prse wldrcbrnse ntssnentxse crsle cttrse wldncbe",
    "alwlp n cbetsme lrse rlse nrglsne as n wldncbe ( nopfse nlsre ncbe ) nte gddmnsen cvrergbrwe",
    "( tene tfrne ncbrtsencbe ing ) ( flrse prse on dej ) ncbe",
    "( cdnse prse dnsoe j4 ncbe )",
    "( prtse prse on rede 75 ncbe )",
    "( tf nscmsp sole mrde luse tote wld n wldncbe )",
    "( 194 wlds ncbe ) ( trfxl )",

    "alpnte glse se erte",

    "vlse mtse ctse wse frtse",

    "pnrtrse ondrse wld nebe",

    "n wld xlrcmsp newld sts me xl",

    "dulmt 6 tunse ncbexe",

    "munsaisten munarse",

    "klse lrste tr se trse mkse",

    "sae 6 nse se n mrse",

    "nmnrcbrnse pte 2ptewsrebrese",

    "26 mlse 74sprkse 29ke nosole",

    "3s 6le clgse senetre dkrse pseshle",

    "6si mtcse htlse ncutctrs nmre",

    "99.84.5 2uneplse ncrse adrtsen srsen se",

    "an mse nrse inr ntrlerc brnse ntsrcrbne",

    "lspnse n gspse mkse r bse ncbeavxlr",

    "hm cre n mre n cbe 1/2 munddlse",

    "xdrlx"
]

cipher_list_2 = [
    "--------1-------"
    "( mnd mknea rse-n-s-m knare ).",
    "tfrne npinse npbser cbbnse nprse inc ."
    "prse nmrse bpre hld enldncbe ( tfxlf tcxl n lbe )",
    "al-prppit xly ppiy ncbe mgksewcd rcbr nse prse ."
    "wldrcbrnse nt ssnentxse crsle citrse wldncbe .",
    "al wlp n cbets me lrse rlse n r glsne as n wldncbe ."
    "( nopfse nlsre ncbe ) nte gddmnsen cu rercbrne .",
    "( tene tfrne ncbrtse ncbe inc ) .",
    "###( flrse prse onde 71 ncbe ).",
    "( cdnse prse onsde 74 ncbe ) .",
    "( prtse prse onrede 75 ncbe ) .###",
    "( tf ?nsc?msp sole mrde luse tote wld n wldn cbe ) .",
    "( 194 wld's ncbe ) ( trfxl ) .",
    "--------21-------",
    "alpnte glse - se erte .",
    "ylse mtse - ctse - wse - frtse .",
    "pnrtrse ondrse wld ncbe .",
    "n wld xlrcmsp newld sts me xl .",
    "dulmt 6 tunse ncbexe .",
    "--------22-------",
    "( munsa?i?sten munarse ).",
    "klse - lrste - tr se - trse - mkse -mrse .",
    "( sae ?6? nse se n mrse ).",
    "--------23-------",
    "nmn ?rcbrnse pte 2p tewsrc ?brese .",
    "26 mlse 74 sprkse ?29 ke nosole 173 rtrse .",
    "3s 6 le clgse sknvtxe dkrse pseshle .",
    "6si mtcse htlse ncutc trs n mre .",
    "99.84.5 2 uneplse n crse aortse n sksenb se .",
    "n sre onse putse wld n cbe ( 3 xorl ) .",
    "--------24-------",
    "n mse n rse inr n trlercb rnse ?n tsrcrs ne? .",
    "lspnse n gspse mkse r bse ?n cbeav? xlr .",
    "hn cre n mre n cbe 1/2mund dlse",
    "--------25-------",
    "d - w - m - 4 ?mil xdrlx ."
]

cipher_list_2_clean = [
    "mnd mknea rse knare",
    "tfrne n pinse npbser cbbnse n prse inc"
    "prse nmrse bpre hld enld ncbe tfxlf tcxl n lbe",
    "al prppit xly ppiy ncbe mgksewcd rcbr nse prse"
    "wldrcbrnse nt ssnentxse crsle citrse wld ncbe",
    "al wlp n cbets me lrse rlse n r glsne as n wld ncbe"
    "nopfse nlsre ncbe nte gddmnsen cu rercbrne",
    "tene tfrne ncbrtse ncbe inc",
    "flrse prse onde ncbe",
    "cdnse prse onsde ncbe",
    "prtse prse onrede ncbe",
    "tf nscmsp sole mrde luse tote wld n wldn cbe",
    "wlds ncbe trfxl",
    "alpnte glse se erte",
    "ylse mtse ctse wse frtse",
    "pnrtrse ondrse wld ncbe",
    "n wld xlrcmsp newld sts me xl",
    "dulmt tunse ncbe xe",
    "munsaisten munarse",
    "klse lrste tr se trse mkse mrse",
    "sae nse se n mrse",
    "nmn rcbrnse pte tewsrc brese",
    "mlse sprkse ke nosole rtrse",
    "s le clgse sknvtxe dkrse pseshle",
    "si mtcse htlse ncutc trs n mre",
    "uneplse n crse aortse n sksenb se",
    "n sre onse putse wld n cbe xorl",
    "n mse n rse inr n trlercb rnse n tsrcrs ne",
    "lspnse n gspse mkse r bse n cbeav xlr",
    "hn cre n mre n cbe mund dlse",
    "mil xdrlx"
]

cipher_list_4lw = [
    "mknea knare wld",
    "tfrne pinse npbser cbbnse prse inc"
    "prse nmrse bpre enld ncbe tfxlf tcxl",
    "prppit ppiy ncbe mgksewcd rcbr prse"
    "wldrcbrnse ssnentxse crsle citrse ncbe",
    "cbets lrse rlse glsne ncbe"
    "nopfse nlsre ncbe gddmnsen rercbrne",
    "tene tfrne ncbrtse ncbe",
    "flrse prse onde ncbe",
    "cdnse prse onsde ncbe",
    "prtse prse onrede ncbe",
    "nscmsp sole mrde luse tote wld n wldn cbe",
    "wlds ncbe trfxl",
    "alpnte glse erte",
    "ylse mtse ctse frtse",
    "pnrtrse ondrse ncbe",
    "xlrcmsp newld",
    "dulmt tunse ncbe",
    "munsaisten munarse",
    "klse lrste trse mkse mrse",
    "mrse",
    "rcbrnse tewsrc brese",
    "mlse sprkse nosole rtrse",
    "clgse sknvtxe dkrse pseshle",
    "mtcse htlse ncutc",
    "uneplse crse aortse sksenb",
    "onse putse xorl",
    "trlercb rnse tsrcrs",
    "lspnse gspse mkse cbeav",
    "mund dlse",
    "xdrlx"
]